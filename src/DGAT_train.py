        ...
        for i in range(num_steps):
            ...
            if (i + 1) % 25 == 0 and verbose == True:
                ...
                if valid_aupr >= best_valid_aupr:
                    ...
                    #     saver = tf.train.Saver()
                    #     print("-----saving0.955-----")
                    #     save_path = saver.save(sess, model_path, global_step=9955)
                    #     print("-----saved-----")
...
for r in range(1):
    ...
    train_data_set = np.load('dataset/' + opts.r + '_train.npy')
    valid_data_set = np.load('dataset/' + opts.r + '_valid.npy')
    ...
    if opts.t == 'unique':
        ...
        DTItrain = train_data_set
        DTIvalid = valid_data_set
        DTItest = test_data_set
        DTItrain, DTIvalid = train_test_split(np.vstack((DTItrain, DTIvalid)), test_size=0.1, random_state=rs)
        ...
        np.savetxt('test_auc_DGAT', test_auc_round)
        np.savetxt('test_aupr_DGAT', test_aupr_round)
