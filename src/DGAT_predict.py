...
# 药物名称
drug_name = np.loadtxt(network_path + 'drug.txt',dtype=str)
# 靶点名称
protein_name = np.loadtxt(network_path + 'protein.txt',dtype=str)
...
def train_and_evaluate(DTItrain, DTIvalid, DTItest, graph, verbose=True, num_steps=4000):
    ...
    positive_counts = nagetive_counts = pre_yes = pre_no = 0
    ...
    for i in range(num_steps):
        ...
        if valid_aupr >= best_valid_aupr:
            ...
            for ele in DTItest:
                print('\ntest_data:', ele)
                print(drug_name[ele[0]], protein_name[ele[1]], ele[2].astype(bool))
                print('predict:', results[ele[0], ele[1]] > 0.5)
                if ele[2] == 0:
                    nagetive_counts += 1
                    if results[ele[0], ele[1]] < 0.5:
                        pre_yes += 1
                    else:
                        pre_no += 1
                else:
        ...
        print('总数：%d 正样本数：%d 负样本数：%d 预测正确数：%d 预测错误数：%d' % (
            positive_counts + nagetive_counts, positive_counts, nagetive_counts, pre_yes, pre_no))
...
for r in range(1):
    ...
    # test_data_set = np.loadtxt('dataset/DGAT_test.txt').astype(int)
    test_data_set = np.load('dataset/' + opts.r + '_test.npy')
