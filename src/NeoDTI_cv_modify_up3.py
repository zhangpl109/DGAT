...
class Model(object):
    ...
    def _build_model(self):
        ...
        if True:
            ...
            # drug_vector = tf.nn.l2_normalize(relu(tf.matmul(concat_drug_pre_add, W0) + b0), dim=1)
            # protein_vector = tf.nn.l2_normalize(relu(tf.matmul(concat_protein_pre_add, W0) + b0), dim=1)
            ...
            drug_vector = tf.nn.l2_normalize(relu(tf.matmul(drug_pool_concat, W00) + b0), dim=1)
            protein_vector = tf.nn.l2_normalize(relu(tf.matmul(protein_pool_concat, W00) + b0), dim=1)
            disease_vector = tf.nn.l2_normalize(relu(tf.matmul(
            ...
            sideeffect_vector = tf.nn.l2_normalize(relu(tf.matmul(
        ...
        if True:
            ...
            drug_vector = tf.nn.l2_normalize(relu(tf.matmul(drug_concat, W1) + b1), dim=1)
            protein_vector = tf.nn.l2_normalize(relu(tf.matmul(protein_concat, W1) + b1), dim=1)
