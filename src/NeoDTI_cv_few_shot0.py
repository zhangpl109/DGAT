...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        self.protein_sequence_normalize = tf.constant(protein_sequence_normalize,tf.float32, [num_protein, num_protein])
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        self.drug_protein = np.zeros((num_drug, num_protein))
        self.mask = np.zeros((num_drug, num_protein))

        for ele in inputs:
            self.drug_protein[ele[0], ele[1]] = ele[2]
            self.mask[ele[0], ele[1]] = 1
        self.protein_drug = self.drug_protein.T

        self.drug_protein_normalize = row_normalize(self.drug_protein, False)
        self.protein_drug_normalize = row_normalize(self.protein_drug, False)


        self.drug_protein = tf.constant(self.drug_protein,tf.float32, [num_drug, num_protein])
        self.drug_protein_normalize = tf.constant(self.drug_protein_normalize,tf.float32, [num_drug, num_protein])

        self.protein_drug = tf.constant(self.protein_drug,tf.float32, [num_protein, num_drug])
        self.protein_drug_normalize = tf.constant(self.protein_drug_normalize,tf.float32, [num_protein, num_drug])

        self.drug_protein_mask = tf.constant(self.mask,tf.float32, [num_drug, num_protein])
...
def train_and_evaluate(DTItrain, DTIvalid, DTItest, graph, verbose=True, num_steps=4000):
    ...
    for i in range(num_steps):
        ...
        results,total_loss,dti_loss = model(DTItrain)
        ...
        # pre_model = model
        # save_path = saver.save(sess, model_path, global_step=2)
        # print(pre_model.predict(DTItest))
        if i % 1 == 0 and verbose == True:
            print('step', i, 'total and dtiloss', total_loss, dti_loss)
